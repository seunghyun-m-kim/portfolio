...
try:
    assert legacy_df.count() == spark.sql(f'SELECT * FROM {catalog}.{schema}.{table}').count()
...
try:
    assert len(legacy_df.collect()[0]) == len(spark.sql(f'SELECT * FROM {catalog}.{schema}.{table}').collect()[0])
...
try:
    ...
    assert (spark.sql(f'SELECT SUM(CRC32({checksum_column})) total FROM legacy_df').collect()[0]['total'] == 
            spark.sql(f'SELECT SUM(CRC32({checksum_column})) total FROM {catalog}.{schema}.{table}').collect()[0]['total'])
